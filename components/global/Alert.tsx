import clsx from "clsx";

type Props = {
  message: string;
  type: "success" | "warning" | "error" | "neutral";
  link?: string;
};

export default function Alert({ message, type, link }: Props) {
  return (
    <div
      className={clsx("p-4 rounded-md", {
        "bg-green-50": type == "success",
        "bg-yellow-50": type == "warning",
        "bg-red-50": type == "error",
        "bg-gray-50": type == "neutral",
      })}
    >
      <div className="flex">
        <div className="flex-shrink-0">
          <svg
            className={clsx("h-5 w-5", {
              "text-green-400": type == "success",
              "text-yellow-400": type == "warning",
              "text-red-400": type == "error",
              "text-gray-400": type == "neutral",
            })}
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
            fill="currentColor"
            aria-hidden="true"
          >
            <path
              fillRule="evenodd"
              d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
              clipRule="evenodd"
            />
          </svg>
        </div>
        <div className="ml-3 flex-1 md:flex md:justify-between">
          <p
            className={clsx("text-sm text-left", {
              "text-green-400": type == "success",
              "text-yellow-400": type == "warning",
              "text-red-400": type == "error",
              "text-gray-400": type == "neutral",
            })}
          >
            {message}
          </p>

          {link && (
            <p className="mt-3 text-sm md:mt-0 md:ml-6">
              <a
                href={link}
                className={clsx("whitespace-nowrap font-medium ", {
                  "text-green-400 hover:text-green-600": type == "success",
                  "text-yellow-400 hover:text-yellow-600": type == "warning",
                  "text-red-400 hover:text-red-600": type == "error",
                  "text-gray-400 hover:text-gray-600": type == "neutral",
                })}
              >
                Details <span aria-hidden="true">&rarr;</span>
              </a>
            </p>
          )}
        </div>
      </div>
    </div>
  );
}
